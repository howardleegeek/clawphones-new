...
log('Starting ClawPhones AI Agent...', 'DEBUG');
...
const AGENT_NAME = config.agentName || 'ClawPhones';
let BRIDGE_TOKEN = normalizeSecret(config.bridgeToken || '');
const USER_AGENT = 'ClawPhones/1.0 (Android; +https://clawphones.ai)';

// ClawPhones Cloud proxy — when set, all Claude API calls route through our billing proxy
// instead of direct to api.anthropic.com. The proxy handles billing + credits.
const PROXY_URL = normalizeSecret(config.proxyUrl || '');   // e.g. "https://api.clawphones.ai"
const PROXY_TOKEN = normalizeSecret(config.proxyToken || ''); // cp_xxxx token from proxy registration
...
const USE_PROXY = !!(PROXY_URL && PROXY_TOKEN);

if (!BOT_TOKEN) {
    log('ERROR: Missing required config (botToken)', 'ERROR');
    process.exit(1);
}
if (!USE_PROXY && !ANTHROPIC_KEY) {
    log('ERROR: Missing required config — set anthropicApiKey (BYOK) or proxyUrl+proxyToken (platform mode)', 'ERROR');
    process.exit(1);
}
if (USE_PROXY) {
    log(`Auth: platform proxy (${PROXY_URL}) | Model: ${MODEL}`, 'DEBUG');
} else {
    log(`Auth: ${AUTH_TYPE === 'setup_token' ? 'setup-token' : 'api-key'} (direct) | Model: ${MODEL}`, 'DEBUG');
}
