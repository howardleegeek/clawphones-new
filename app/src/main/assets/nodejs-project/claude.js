...
function classifyApiError(status, data) {
    ...
    if (status === 402) {
        const isProxy = !!(PROXY_URL && PROXY_TOKEN);
        const detail = data?.detail;
        const remaining = typeof detail === 'object' ? detail.available : null;
        return {
            type: 'billing', retryable: false,
            userMessage: isProxy
                ? `💳 Credits exhausted${remaining !== null ? ` (${remaining} remaining)` : ''}. Top up in ClawPhones Settings → Billing.`
                : 'Your API account needs attention — check billing at console.anthropic.com'
        };
    }
    if (status === 429) {
...
async function claudeApiCall(body, chatId, traceCtx = {}) {
    ...
    try {
        // Determine target: proxy (ClawPhones Cloud) or direct Anthropic
        const useProxy = !!(PROXY_URL && PROXY_TOKEN);
        let targetHostname, targetPort, targetPath, requestHeaders;

        if (useProxy) {
            // Route through ClawPhones billing proxy
            const proxyUrl = new URL(PROXY_URL);
            targetHostname = proxyUrl.hostname;
            targetPort = proxyUrl.port ? parseInt(proxyUrl.port) : 443;
            ...
            // Direct Anthropic API (user's own key)
