...
data class AppConfig(
    ...
    val authType: String = "api_key", // "api_key", "setup_token", or "platform"
    ...
    val activeCredential: String
        get() = when (authType) {
            ...
            "platform" -> proxyToken  // Platform mode uses proxy token
            else -> anthropicApiKey
        }

    /** Whether this config uses platform proxy mode (no BYOK needed). */
    val isPlatformMode: Boolean
        get() = authType == "platform" && proxyUrl.isNotBlank() && proxyToken.isNotBlank()
}
...
object ConfigManager {
    ...
    private const val KEY_PROXY_TOKEN_ENC = "proxy_token_enc"
    ...
    fun saveConfig(context: Context, config: AppConfig) {
        ...
        // Proxy config (platform mode)
        editor.putString(KEY_PROXY_URL, config.proxyUrl)
        if (config.proxyToken.isNotBlank()) {
            val encProxy = KeystoreHelper.encrypt(config.proxyToken)
            editor.putString(KEY_PROXY_TOKEN_ENC, Base64.encodeToString(encProxy, Base64.NO_WRAP))
        } else {
            editor.remove(KEY_PROXY_TOKEN_ENC)
        }
    ...
    fun loadConfig(context: Context): AppConfig? {
        ...
        val proxyToken = try {
            val enc = p.getString(KEY_PROXY_TOKEN_ENC, null)
            if (enc != null) KeystoreHelper.decrypt(Base64.decode(enc, Base64.NO_WRAP)) else ""
        } catch (e: Exception) {
            Log.w(TAG, "Failed to decrypt proxy token", e)
            LogCollector.append("[Config] Failed to decrypt proxy token: ${e.javaClass.simpleName}", LogLevel.ERROR)
            ""
        }
        ...
        return AppConfig(
            ...
            model = p.getString(KEY_MODEL, "free-nemotron") ?: "free-nemotron",
            ...
            proxyUrl = p.getString(KEY_PROXY_URL, "") ?: "",
    ...
    fun updateConfigField(context: Context, field: String, value: String) {
        ...
        val updated = when (field) {
            ...
            "proxyUrl" -> config.copy(proxyUrl = value)
            "proxyToken" -> config.copy(proxyToken = value)
    ...
    fun writeConfigJson(context: Context, bridgeToken: String) {
        ...
        val proxyField = if (config.proxyUrl.isNotBlank() && config.proxyToken.isNotBlank()) {
            """,
            |  "proxyUrl": "${escapeJson(config.proxyUrl)}",
            |  "proxyToken": "${escapeJson(config.proxyToken)}""""
        } else ""
        val json = """
            |{
            ...
            |  "bridgeToken": "${escapeJson(bridgeToken)}"$proxyField$braveField$jupiterField$mcpField
            |}
        """.trimMargin()
