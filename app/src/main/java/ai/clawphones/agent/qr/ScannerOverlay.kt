package ai.clawphones.agent.qr
...
private fun ScannerOverlayCanvas(
    ...
    val vignetteColor = ClawPhonesColors.Background.copy(alpha = 0.7f)
    val bracketColor = ClawPhonesColors.Primary
    val scanLineColor = ClawPhonesColors.Primary.copy(alpha = 0.6f)
    val scanGlowColor = ClawPhonesColors.Primary.copy(alpha = 0.15f)
    val crosshairColor = ClawPhonesColors.TextDim.copy(alpha = 0.4f)
...
private fun ScannerTopBar(
    ...
    Row(
        modifier = modifier
            .fillMaxWidth()
            .background(ClawPhonesColors.Background.copy(alpha = 0.85f))
            .statusBarsPadding()
        ...
        if (torchAvailable) {
            IconButton(onClick = onToggleTorch) {
                Icon(
                    ...
                    tint = if (torchEnabled) ClawPhonesColors.Warning else ClawPhonesColors.TextDim,
                )
            }
        } else {
...
private fun ScannerBottomStatus(
    ...
    Column(
        ...
        Text(
            ...
            fontFamily = FontFamily.Monospace,
            fontSize = 13.sp,
            color = if (isDetected) ClawPhonesColors.Accent else ClawPhonesColors.TextSecondary,
            letterSpacing = 1.sp,
        )
