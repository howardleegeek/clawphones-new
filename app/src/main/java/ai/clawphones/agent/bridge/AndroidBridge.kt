package ai.clawphones.agent.bridge
...
import ai.clawphones.agent.camera.CameraCaptureActivity
...
class AndroidBridge(
    ...
    private fun handleSolanaAuthorize(): Response {
        val requestId = java.util.UUID.randomUUID().toString()
        val intent = Intent(context, ai.clawphones.agent.solana.SolanaAuthActivity::class.java).apply {
    ...
    private fun handleSolanaSignOnly(params: JSONObject): Response {
        ...
        val resultsDir = java.io.File(context.filesDir, ai.clawphones.agent.solana.SolanaAuthActivity.RESULTS_DIR)
